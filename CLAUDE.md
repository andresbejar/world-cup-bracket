...
>
> **The tournament is over and this project has been wound down.** The
> deployed site is a frozen, public, read-only artifact: all pages
> prerender to static HTML from `data/archive-snapshot.json`, and it runs
> with no database, no auth, and no environment variables.
>
> **Most of the document below describes the retired live system.** Treat
> it as a record of how it worked, not as instructions. Specifically, these
> no longer exist: the score-polling cron and its external pinger, the
> nightly backup workflow, the Playwright E2E suite, every `/api/*` route,
> Google sign-in, and the prize-pool claim flow. The Supabase projects, the
> api-sports key and the remaining GitHub secrets are decommissioned
> separately, after this lands and the static archive is verified in
> production.
>
> **Still accurate and still authoritative:** `DESIGN.md`, the pure-function
> rule for `lib/bracket.ts` and its coverage bar, the slot-ID prediction
> model, and the scoring rules.
>
> The archive's own data layer is `lib/archive.ts`, generated by
> `scripts/build-archive-snapshot.ts`. See `README.md` for the wind-down
> and how the static archive is produced.
