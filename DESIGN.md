...
- **Match row:** card with a mono meta header (match #, group/round, kickoff time on the left; save status on the right), then two stacked team rows — each row ends in its own score stepper so the team↔score pairing reads straight across at any viewport width. Optional inline penalty-winner picker for tied knockout scores. Never split teams and score inputs into separate columns. Locked rounds (and unresolved knockout matchups) freeze the stepper: the chevrons unmount, the value renders read-only, and the status label reads `LOCKED` — no opacity layering. Unpicked scores render a dim `–` placeholder, never a fake 0 (users read 0:0 as a saved draw); the first `+` tap materializes a real 0, so a 0-0 pick is one tap.
